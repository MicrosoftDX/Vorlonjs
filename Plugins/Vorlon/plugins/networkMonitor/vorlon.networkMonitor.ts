    ...
    export class NetworkMonitor extends Plugin {
        ...
        public sendClientData(): void {
            ...
            for (var i = 0; i < entries.length; i++) {
                ...
                item.type = entries[i].initiatorType;
                item.startTime = entries[i].startTime;
                item.duration = entries[i].duration;
                item.redirectStart = entries[i].redirectStart;
                item.redirectDuration = entries[i].redirectEnd - entries[i].redirectStart;
                item.dnsStart = entries[i].domainLookupStart;
                item.dnsDuration = entries[i].domainLookupEnd - entries[i].domainLookupStart;
                item.tcpStart = entries[i].connectStart;
                item.tcpDuration = entries[i].connectEnd - entries[i].connectStart; 	// TODO
                item.requestStart = entries[i].requestStart;
                item.requestDuration = entries[i].responseStart - entries[i].requestStart;
                item.responseStart = entries[i].responseStart;
                item.responseDuration = (entries[i].responseStart == 0 ? 0 : entries[i].responseEnd - entries[i].responseStart);
